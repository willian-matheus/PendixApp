 ...
 *
 * Uma pendência recorrente não gera todas as ocorrências de uma vez: quando a
 * atual é marcada como recebida, o app cria a ocorrência seguinte a partir
 * dela (ver `gerarProximaOcorrencia` em services/pendix.ts). Assim a fila
 * nunca enche de pendências futuras e a competência caminha sozinha.
 *
 * Este arquivo é de propósito 100% puro (sem React Native, sem Supabase) para
 * poder rodar direto no Node — ver lib/periodicidade.test.ts.
 */
...
/** O "a cada quanto" que aparece ao lado do rótulo no Select. */
...
// Rótulo curto no chip; o "a cada quanto" aparece como legenda do escolhido
// (11 opções com a descrição inteira dentro do chip viram uma parede de texto).
export const PERIODICIDADE_OPTS: { value: PendixPeriodicidade; label: string }[] =
  (Object.keys(PERIODICIDADE_LABEL) as PendixPeriodicidade[]).map((value) => ({
    value,
    label: value === 'unica' ? 'Única' : PERIODICIDADE_LABEL[value],
  }));

/** Só as que repetem — é o que faz sentido oferecer para cobrança automática. */
export const FREQUENCIA_COBRANCA_OPTS = PERIODICIDADE_OPTS.filter((o) => o.value !== 'unica');
...
export function somarDias(iso: string, dias: number): string {
  ...
  // deslocar o dia (mesmo cuidado da Edge Function notificar-pendencias).
